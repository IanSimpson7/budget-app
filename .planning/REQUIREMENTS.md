...
- [x] **FOOD-01**: App reads `meal_pool.md` from the SMC project (`../schedule-meal-coordinator/`) read-only; never writes to or triggers SMC
...
- [x] **FOOD-03**: App reads `plans/<date>.md` (and `plans/<start>--<end>.md`) from the SMC project to determine which meals are scheduled for which dates
...
- [x] **FOOD-06**: Meal cost is computed as Σ over macro-bearing ingredients (portion × unit_cost); meal costs are NEVER hard-coded
- [x] **FOOD-07**: Period food floor is computed as Σ over meals scheduled in the period; recomputes when pool, plan, unit-cost map, or portion model changes
...
- [x] **FOOD-09**: New meals built from already-priced ingredients price automatically without user action
...
- [x] **FOOD-11**: When no current meal plan exists (SMC hasn't run, or there's a gap), food floor falls back to the last-known value or a high-water mark — NEVER a lower number. Staleness is flagged in UI.
...
| FOOD-07 | Phase 4 | Complete |
| FOOD-08 | Phase 4 | Pending |
| FOOD-09 | Phase 4 | Complete |
| FOOD-10 | Phase 4 | Complete |
| FOOD-11 | Phase 4 | Complete |
